...
class Start(Frame):
    def __init__(self, parent, controller):
        ...
        textFont = tkfont.Font(family="Times New Roman", size=10, weight= "bold" )

        text = Label(self, text='Please choose a character')
        text.configure(font=my_font)
        text.place(relx=0.5, rely=0.1, anchor=CENTER)

        self.btn_woman_bg = ImageTk.PhotoImage(Image.open(os.path.join(self.controller.dirname, '../Assets/btn_woman.jpg')))
        ...
        text_female = textwrap.fill('This is Lisa. She used to work as a preschool teacher. After she lost both of her children, she got depressed and lost everything she had. Do you want to help Lisa get back on track?', width=50)
        text_female = Label(self, width=50, text=text_female)
        text_female.configure(font=textFont)
        text_female.place(relx=0.25, rely=0.6, anchor=CENTER)

        text_male = textwrap.fill('This is Edward. He fought hard for his country. But the PTSD after the things he has been through wouldnt leave him alone. He needs your help to get his house back. Will you help him?', width=50)
        text_male = Label(self, width=50, text=text_male)
        text_male.configure(font=textFont)
